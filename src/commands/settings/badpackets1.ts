...
function badpackets1Help(player: Player, prefix: string, badPackets1Boolean: boolean, setting: boolean): void {
    ...
    sendMsgToPlayer(player, [
        ...
        `    -s, --status    Display the current status of Badpackets1`,
        `    -e, --enable    Enable Badpackets1`,
        `    -d, --disable   Disable Badpackets1`,
        `§4[§6Description§4]§f: Toggles checks for message lengths with each broadcast.`,
        `§4[§6Examples§4]§f:`,
        `    ${prefix}badpackets1 --help`,
        `    ${prefix}badpackets1 --status`,
        `    ${prefix}badpackets1 --enable`,
        `    ${prefix}badpackets1 --disable`,
    ]);
}
    ...
    if (args.length > 0) {
        ...
        switch (additionalArg) {
            ...
            default:
                ...
                sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f Invalid option. Use ${prefix}badpackets1 --help for more information.`);
                break;
        }
    } else {
